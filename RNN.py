...
def get_compiled_rnn():
    model = tf.keras.Sequential([
        
        tf.keras.layers.LSTM(45, activation='relu', input_shape=(n_steps, n_features),kernel_regularizer=regularizers.l2(0.001),return_sequences=False),
        layers.Dropout(0.3),
...
rnn=get_compiled_rnn()
history = rnn.fit(X, y ,validation_data=(X1, y1),batch_size=(30-n_steps+1-5),epochs=50)
...
plt.ylabel('value')
plt.xlabel('epoch')
plt.legend(['train-rmse', 'val-rmse','train-loss','val-loss'], loc='upper right')
plt.show()

_ ,rmse=rnn.evaluate(X4,y4)
print("2016 rmse=",rmse)

_ ,rmse=rnn.evaluate(X2,y2)
print("2017 rmse=",rmse)

_ ,rmse=rnn.evaluate(X3,y3)
