...
cnn=get_compiled_cnn()

history = cnn.fit(X, y ,validation_data=(X1, y1),batch_size=(30-n_steps+1-5),epochs=100)
summ=cnn.summary()
...
plt.ylabel('accuracy')
plt.xlabel('epoch')
plt.legend(['train-rmse', 'val-rmse','train-loss','val-loss'], loc='upper left')
plt.show()

_ ,rmse=cnn.evaluate(X4,y4)
print("2016 rmse=",rmse)

_ ,rmse=cnn.evaluate(X2,y2)
print("2017 rmse=",rmse)

_ ,rmse=cnn.evaluate(X3,y3)
